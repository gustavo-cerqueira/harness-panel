 ...
 *
 * `match` is LITERAL text, never a regular expression. The pattern and the
 * names it runs against both come out of the same inventoried repository, so a
 * regex engine would let that repository hand the reader a catastrophically
 * backtracking pattern and hang the tab on page load. Screening patterns for
 * that is a losing game; literal matching removes the weapon, and every real
 * entry was a plain substring anyway.
 */
...
/**
 * Whether a curated bypass applies to a scanned row, by name. Case-insensitive
 * substring, deliberately: see the note on `match` above. The browser keeps its
 * own copy of this rule, because it cannot import from here.
 */
export function matchesBypass(entry, name) {
	if (!entry || typeof entry.match !== 'string' || !entry.match) return false;
	return String(name ?? '')
		.toLowerCase()
		.includes(entry.match.toLowerCase());
}
 ...
 *   bypasses: object[], ownerOnlyKeys: string[],
 *   clusters: {skills: object, commands: object, agents: object, mcp: object}}}
 */
export function readCuration({ projectRoot } = {}) {
	...
	// Which settings only their owner may change is a policy, not something a
	// settings file states. It used to be a hardcoded regex naming one
	// workspace's env vars: badged on every machine, and nobody else's real
	// policy badged at all.
	const ownerOnlyKeys = (Array.isArray(json.ownerOnlyKeys) ? json.ownerOnlyKeys : []).filter((entry, index) => {
		if (typeof entry === 'string' && entry.trim()) return true;
		warnings.push(`ownerOnlyKeys[${index}] is not a non-empty string and was dropped.`);
		return false;
	});
