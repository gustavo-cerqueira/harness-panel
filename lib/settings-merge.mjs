...
/**
 * Whether a key is one only its owner may change.
 *
 * Which keys those are is a POLICY of the repository being inventoried, not
 * something a settings file states — it was a regex naming one workspace's
 * model env vars, hardcoded here, which badged those names on every machine and
 * nobody else's real policy anywhere. The names now come from that repository's
 * `.claude/harness-curation.json` (`ownerOnlyKeys`), matched as literal
 * case-insensitive substrings against the leaf and the full dotted path.
 */
export function isOwnerOnlyKey(key, ownerOnlyKeys = []) {
	if (typeof key !== 'string' || key.length === 0 || !ownerOnlyKeys.length) return false;
	const leaf = key.slice(key.lastIndexOf('.') + 1).toLowerCase();
	const full = key.toLowerCase();
	return ownerOnlyKeys.some((needle) => {
		const value = String(needle).toLowerCase();
		return leaf.includes(value) || full.includes(value);
	});
}
...
export function mergeSettings(options) {
	const { home, projectRoot, curation } = options ?? {};
	const ownerOnlyKeys = curation?.ownerOnlyKeys ?? [];
	...
	for (const [key, bucket] of [...byKey.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
		...
		if (union) {
			...
			keys.push({
				...
				ownerOnly: isOwnerOnlyKey(key, ownerOnlyKeys),
