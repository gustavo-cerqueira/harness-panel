...
/** Whether anything at all sits at this path, symlink included, dangling or not. */
function occupiesPath(absPath) {
	try {
 ...
 */
/**
 * Where to look when nobody said where.
 *
 * `~/projects` is a guess about one machine's layout, and for anyone who keeps
 * repositories somewhere else it produced a selector holding a single entry and
 * no hint that a setting existed. So when that directory is not there, the
 * neighbours of the repository the panel was started in are searched instead —
 * which is where the rest of somebody's work almost always sits.
 *
 * Two neighbourhoods are refused outright: the home directory, and the
 * filesystem root. A repository checked out directly in either would otherwise
 * turn the whole thing into a search base and send the panel walking it.
 */
function defaultBases({ home, extraRoots }) {
	const hasHome = typeof home === 'string' && home.length > 0;
	const projects = hasHome ? path.join(home, 'projects') : null;
	if (projects) {
		const resolvedProjects = realpathOrNull(projects);
		if (resolvedProjects && isDirectorySafe(resolvedProjects)) return [resolvedProjects];
		// Something occupies the path but will not resolve — a symlink to an
		// unmounted drive, a file where a directory belongs. That is a broken
		// convention, not an absent one, and quietly searching somewhere else
		// instead would widen what the panel reads while saying nothing. Hand it
		// back so the caller reports it as the unusable base it is.
		if (occupiesPath(projects)) return [projects];
	}

	const started = (Array.isArray(extraRoots) ? extraRoots : []).find(
		(entry) => typeof entry === 'string' && entry.length > 0,
	);
	if (started) {
		const parent = realpathOrNull(path.dirname(path.resolve(started)));
		const resolvedHome = hasHome ? realpathOrNull(home) : null;
		const isFilesystemRoot = parent !== null && path.dirname(parent) === parent;
		if (parent && !isFilesystemRoot && parent !== resolvedHome && isDirectorySafe(parent)) return [parent];
	}

	// Nothing better to offer. Naming the missing default still tells the reader
	// which directory the panel looked for and did not find.
...
export function discoverWorkspaceRoots({ home, bases, extraRoots } = {}) {
	...
	const baseInputs = explicitBases.length > 0 ? explicitBases : defaultBases({ home, extraRoots });
	...
	// Always, and after the bases: what the caller handed over is the one root
	// it is certain about, and it must survive a discovery that found no base
	// to search — which is why this is not inside the loop above.
	...
	return {
		bases: baseResults,
		roots,
		error: baseInputs.length === 0 && roots.length === 0 ? 'no home or bases given' : null,
	};
}
